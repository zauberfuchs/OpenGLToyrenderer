...
int main() {
	...
	g_Camera = CreateRef<Camera>(glm::vec3(10.0f, 4.0f, -1.0f), glm::vec3(0.0f, 1.0f, 0.0f), 180.0f, -25.0);
	...
	auto wheelTexture = CreateRef<Texture>("../Data/Textures/wheel_bump.png", TextureTarget::Texture2D);
	wheelTexture->SetTextureTarget(TextureTarget::Texture2D);
	wheelTexture->SetUniformLocation("material.albedoMap");

	auto basketballTexture = CreateRef<Texture>("../Data/Textures/basketball.png", TextureTarget::Texture2D);
	...
	const auto wheelMaterial = CreateRef<Material>("wheel");
	...
	const auto basketballMaterial = CreateRef<Material>("basketball");
	...
	const auto greyMaterial = CreateRef<Material>("grey");
	...
	auto plane = CreateRef<SceneObject>("plane");
	plane->AddModel(CreateRef<Model>(std::string("plane")));
	plane->GetModel()->AddMesh(CreateRef<Plane>("plane"));
	plane->GetModel()->GetMesh()->SetMaterial(greyMaterial);
	plane->GetTransform()->Scale(glm::vec3(20.0f, 20.0f, 20.0f));
	
	auto wheel = CreateRef<SceneObject>("wheel");
	wheel->AddModel(CreateRef<Model>("../Data/Models/wheel/Autoreifen.obj"));
	wheel->GetModel()->GetMesh()->SetMaterial(wheelMaterial);
	wheel->GetTransform()->Translate(glm::vec3(0.0f, 0.4f, 0.0f), Space::Local);
	
	auto basketball = CreateRef<SceneObject>("basketball");
	basketball->AddModel(CreateRef<Model>("../Data/Models/basketball/Basketball.obj"));
	basketball->GetModel()->GetMesh()->SetMaterial(basketballMaterial);
